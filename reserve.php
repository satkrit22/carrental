...
<div class="col-md-7">
    <div class="form-group">
        <label for="car_search">Search for a Car:</label>
        <input type="text" class="form-control" id="car_search" placeholder="Enter car name, brand, or type">
    </div>
    <div class="items_tab">
...
$(document).ready(function() {
    $('#car_search').on('input', function() {
        ...
        $.ajax({
            type: 'POST',
            url: 'search_cars.php', // Create this PHP file to handle search
            data: { search_term: searchTerm },
            success: function(response) {
                $('.items_tab').html(response);
            }
        });
    });
});
